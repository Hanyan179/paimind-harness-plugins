...
function descriptor(method: string, input: z.ZodType | undefined, output: z.ZodType) {
  return Object.freeze({
    id: `@paimind/mcp-center#paimindMcpConnections/${method}`,
    service: 'paimindMcpConnections', namespace: 'paimindMcpConnections', method,
    invocation: Object.freeze({ kind: 'direct' as const }),
    parameters: Object.freeze(input === undefined ? [] : [{ name: 'input', wire: 'input', source: 'json' as const, codec: { mode: 'strict' as const, typeSymbol: `@paimind/mcp-center#${method}Input`, schema: input } }]),
    result: Object.freeze({ mode: 'strict' as const, typeSymbol: `@paimind/mcp-center#${method}Result`, schema: output }),
...
export const TYPERT_REMOTE = Object.freeze({ package: '@paimind/mcp-center', descriptors: MCP_REMOTE_DESCRIPTORS })
