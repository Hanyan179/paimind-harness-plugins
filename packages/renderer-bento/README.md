...
Isolated-origin Bento preview plugin for PAIMind artifacts.

The workbench is a hidden on-demand Better Sidebar tab. Opening an exact Bento
Artifact reveals it with the Artifact title; there is no permanent “Bento
Preview” navigation entry.

The workbench exposes icon-only Preview, Edit, and Trace mode buttons with
accessible hover/focus tooltips. Edit mode enables only renderer-declared
presentation copy; evidence-backed facts and derived metrics stay locked.
...
- `pnpm exec tsc -b packages/renderer-bento/tsconfig.json --pretty false`
- `pnpm exec vitest run packages/renderer-bento/tests`
