...
const OUTLINE_PRODUCERS = new Set([OUTLINE_GENERATOR_PROVIDER_ID, 'paimind.walmart.buyer-proposal-outline'])
...
(()=>{const slides=[...document.querySelectorAll('.slide')];let index=0;const emit=(type,extra={})=>parent.postMessage({type,mode:'preview',slide:index+1,slideId:slides[index]?.dataset.slideId,...extra},'*');const announceSlides=()=>emit('paimind:bento-manifest',{slides:slides.map((slide,slideIndex)=>({slideId:slide.dataset.slideId,title:slide.querySelector('h1')?.textContent?.trim()||'Slide '+(slideIndex+1)}))});const show=next=>{index=(next+slides.length)%slides.length;slides.forEach((slide,i)=>slide.classList.toggle('active',i===index));emit('paimind:bento-slide')};addEventListener('keydown',event=>{if(event.key==='ArrowRight'||event.key===' '){event.preventDefault();show(index+1)}if(event.key==='ArrowLeft'){event.preventDefault();show(index-1)}if(event.key==='Escape')emit('paimind:bento-exit')});addEventListener('message',event=>{if(event.source!==parent)return;const data=event.data;if(!data||typeof data!=='object')return;if(data.type==='paimind:bento-mode'){announceSlides();emit('paimind:bento-ready')}if(data.type!=='paimind:bento-navigate')return;const byId=slides.findIndex(slide=>slide.dataset.slideId===data.slideId);const byNumber=Number.isInteger(data.slide)?data.slide-1:-1;const next=byId>=0?byId:byNumber;if(next>=0&&next<slides.length)show(next)});announceSlides();emit('paimind:bento-ready')})();
</script></body></html>`
}
...
function renderElement(element: PresentationElement): string {
  ...
  const locked = editable === '' ? ' data-edit-lock="fact" title="数据已锁定，请从原始资料或分析结果更新"' : ''
...
</style><meta name="paimind:presentation-schema" content="${metadata.schema}"><meta name="paimind:presentation-template" content="${escapeHtml(outline.design.templateId)}"><script type="application/json" id="paimind-presentation-spec">${metadataJson}</script></head><body data-mode="preview" data-paimind-mode-contract="exclusive-selection-v1" data-template-id="${escapeHtml(outline.design.templateId)}" data-style-preset="${escapeHtml(outline.design.stylePreset)}" data-density="${escapeHtml(outline.design.density)}"><main class="deck" aria-label="${escapeHtml(outline.title)}">${pages}<p class="trace-status" role="status" aria-live="polite"><span>正在查找来源…</span></p></main><p class="hint hint-preview">← / → · 切换幻灯片</p><p class="hint hint-edit">可试改标题与说明，刷新后恢复；数据与计算结果已锁定</p><script>
(()=>{
...
const emit=(type,extra={})=>parent.postMessage({type,mode,slide:index+1,slideId:slides[index]?.dataset.slideId,...extra},'*')
const announceSlides=()=>emit('paimind:bento-manifest',{slides:slides.map((slide,slideIndex)=>({slideId:slide.dataset.slideId,title:slide.querySelector('h1')?.textContent?.trim()||'Slide '+(slideIndex+1)}))})
const clearSelection=()=>document.querySelectorAll('.selected,.trace-pending').forEach(node=>node.classList.remove('selected','trace-pending'))
const preservePendingSelection=()=>{selectionToken+=1;document.body.dataset.traceStatus='';document.querySelectorAll('.trace-pending').forEach(node=>{node.classList.remove('trace-pending');node.classList.add('selected')})}
const show=next=>{selectionToken+=1;document.body.dataset.traceStatus='';clearSelection();index=(next+slides.length)%slides.length;slides.forEach((slide,i)=>slide.classList.toggle('active',i===index));emit('paimind:bento-slide')}
const select=el=>{const object=el.closest('[data-object-id]');const payload={objectId:object?.dataset.objectId,factId:el.dataset.factId,selector:selectorFor(el)};if(mode!=='trace'){selectionToken+=1;document.body.dataset.traceStatus='';clearSelection();el.classList.add('selected');emit('paimind:bento-select',payload);return}const token=++selectionToken;clearSelection();el.classList.add('trace-pending');document.body.dataset.traceStatus='active';const finish=()=>{if(token!==selectionToken)return;el.classList.remove('trace-pending');el.classList.add('selected');document.body.dataset.traceStatus='';emit('paimind:bento-select',payload)};if(matchMedia('(prefers-reduced-motion: reduce)').matches)finish();else setTimeout(finish,820)}
addEventListener('click',event=>{const target=event.target instanceof Element?event.target:null;if(target===null||mode==='preview'||(mode==='edit'&&target.closest('[data-editable="true"]')))return;const el=target.closest('[data-selector-kind]');if(el)select(el)})
addEventListener('keydown',event=>{if(event.target.closest?.('[contenteditable="true"]'))return;if(event.key==='ArrowRight'||event.key===' '){event.preventDefault();show(index+1)}if(event.key==='ArrowLeft'){event.preventDefault();show(index-1)}if(event.key==='Escape')emit('paimind:bento-exit')})
addEventListener('message',event=>{if(event.source!==parent)return;const data=event.data;if(!data||typeof data!=='object')return;if(data.type==='paimind:bento-mode'&&['preview','edit','trace'].includes(data.mode)){if(data.mode==='preview'){selectionToken+=1;document.body.dataset.traceStatus='';clearSelection()}else if(data.mode==='edit')preservePendingSelection();mode=data.mode;document.body.dataset.mode=mode;document.querySelectorAll('[data-editable="true"]').forEach(el=>{el.contentEditable=mode==='edit'?'true':'false';el.spellcheck=mode==='edit'});announceSlides();emit('paimind:bento-ready')}if(data.type==='paimind:bento-navigate'){const byId=slides.findIndex(slide=>slide.dataset.slideId===data.slideId);const byNumber=Number.isInteger(data.slide)?data.slide-1:-1;const next=byId>=0?byId:byNumber;if(next>=0&&next<slides.length)show(next)}if(data.type==='paimind:bento-focus'){const slideIndex=slides.findIndex(slide=>slide.dataset.slideId===data.slideId);if(slideIndex>=0)show(slideIndex);const root=document.querySelector('[data-object-id="'+data.objectId+'"]');if(root){const target=data.selector?.kind==='chart-point'?root.querySelector('[data-series-key="'+data.selector.seriesKey+'"][data-category-key="'+data.selector.categoryKey+'"]'):data.selector?.kind==='table-cell'?root.querySelector('[data-row-key="'+data.selector.rowKey+'"][data-column-key="'+data.selector.columnKey+'"]'):root;target?.focus();if(target)select(target)}}})
announceSlides();emit('paimind:bento-ready')
})();
  ...
  async generate(input, context) {
    if (input.blueprint !== undefined) {
      ...
      await context.runWorkspaceCommand({
        command: [
          process.execPath, `${runtimeDir()}/hydrate-outline.mjs`, '--blueprint', sidecarPath,
      ...
      await context.runWorkspaceCommand({
        command: [
          process.execPath, `${runtimeDir()}/validate-fact-set.mjs`, 'validate', '--outline', value.filePath,
          '--fact-set', value.factSetPath, '--fact-set-artifact-id', value.factSetArtifactId,
        ].map(shellQuote).join(' '),
...
export const traceableBentoProvider: PaimindGeneratorProvider = {
  ...
  describe(input) { const value = traceableArgs(input); return { path: value.filePath, title: value.outline?.title ?? '可追溯演示' } },
  async generate(input, context) {
    const value = traceableArgs(input)
    if (value.outline === undefined && value.outlinePath !== undefined) {
      const renderCommand = [
        process.execPath, `${runtimeDir()}/render-exact-outline.mjs`,
        '--outline', value.outlinePath, '--outline-artifact-id', value.outlineArtifactId as string,
      ]
      if (value.factSetPath !== undefined) renderCommand.push(
        '--fact-set', value.factSetPath, '--fact-set-artifact-id', value.factSetArtifactId as string,
      )
      renderCommand.push('--output', value.filePath)
      const result = await context.runWorkspaceCommand({
        command: renderCommand.map(shellQuote).join(' '),
    ...
    if (value.outlinePath !== undefined) {
      const command = [process.execPath, `${runtimeDir()}/validate-fact-set.mjs`, 'read', '--outline', value.outlinePath]
...
function currentArtifact(ctx: GeneratorBentoHostContext, exec: PaimindToolRunContext, artifactId: string, producerId: string, suffix: string): Readonly<ArtifactProducedEnvelopeV1> {
  ...
  return requireCurrentSessionArtifact(
    ...
    { description: `the required available current-Session ${suffix} Artifact`, kind: 'json', producerIds: [producerId], pathSuffix: suffix },
  )
}

function currentOutlineArtifact(ctx: GeneratorBentoHostContext, exec: PaimindToolRunContext, artifactId: string): Readonly<ArtifactProducedEnvelopeV1> {
  if (exec.agent === undefined) throw new Error('traceable Bento generation requires a live Harness Agent')
  return requireCurrentSessionArtifact(
    ctx.sessionProjections.snapshot(exec.agent.session).values['paimind.artifacts'], exec, artifactId,
...
export function apply(ctx: GeneratorBentoHostContext): void {
  ...
  ctx.systemPrompt.section({
    ...
    text: 'For a traced presentation, render the primary Bento delivery with generate_traceable_bento_from_outline and the exact current-Session Outline Artifact ID. Pass the exact Fact Set Artifact ID when the Outline is Fact Set-backed; trusted complete Outline producers can render without one. For a Fact Set-backed deck, author only a compact paimind.presentation-outline-blueprint/v1 with story structure and exact Fact IDs, then use create_fact_bound_presentation_outline before Bento generation. The native Tools hydrate canonical evidence and publish the Bento, Trace and Validation Artifacts. Use generate_bento_artifact only for lightweight untraced cards. Never write HTML, generate an unrequested PPTX or substitute PPTX for Bento, alter verified facts, invent source hashes, or invent CSS. An editable PPTX may be generated separately only after Bento succeeds and only when the current user explicitly requests that export.',
  })
  ctx.tools.register(definePaimindHarnessTool({
    ...
    async execute(toolArgs, exec) {
      ...
      return await ctx.paimindArtifactGenerators.execute(OUTLINE_GENERATOR_PROVIDER_ID, { ...toolArgs, __fact_set_path: artifactWorkspaceRelativePath(factSet.path, exec, { label: 'resolved Fact Set Artifact path', pathSuffix: '.fact-set.json' }) }, exec)
    },
  ...
  ctx.tools.register(definePaimindHarnessTool({
    ...
    description: 'Resolve one exact current-Session Outline Artifact and its exact Fact Set Artifact when present, then render and publish the final traceable Bento deck without asking the model to copy the full Outline.',
    parameters: { file_path: { type: 'string', required: true }, outline_artifact_id: { type: 'string', required: true }, fact_set_artifact_id: { type: 'string' } },
    ...
    async execute(toolArgs, exec) {
      ...
      const factSetArtifactId = typeof toolArgs.fact_set_artifact_id === 'string' && toolArgs.fact_set_artifact_id.trim() !== ''
        ? toolArgs.fact_set_artifact_id.trim()
        : undefined
      const outlineArtifact = currentOutlineArtifact(ctx, exec, outlineArtifactId)
      const factSetArtifact = factSetArtifactId === undefined ? undefined : currentArtifact(ctx, exec, factSetArtifactId, 'paimind.fact-layer', '.fact-set.json')
      return await ctx.paimindArtifactGenerators.execute(TRACEABLE_BENTO_GENERATOR_PROVIDER_ID, {
        ...toolArgs,
        __outline_path: artifactWorkspaceRelativePath(outlineArtifact.path, exec, { label: 'resolved Outline Artifact path', pathSuffix: '.outline.json' }),
        ...(factSetArtifact === undefined ? {} : {
          __fact_set_path: artifactWorkspaceRelativePath(factSetArtifact.path, exec, { label: 'resolved Fact Set Artifact path', pathSuffix: '.fact-set.json' }),
  ...
  ctx.tools.register(definePaimindHarnessTool({
    ...
    async execute(toolArgs, exec) {
      ...
      return await ctx.paimindArtifactGenerators.execute(TRACEABLE_BENTO_GENERATOR_PROVIDER_ID, { ...toolArgs, __outline_path: artifactWorkspaceRelativePath(outlineArtifact.path, exec, { label: 'resolved Outline Artifact path', pathSuffix: '.outline.json' }), ...(factSetArtifact === undefined ? {} : { __fact_set_path: artifactWorkspaceRelativePath(factSetArtifact.path, exec, { label: 'resolved Fact Set Artifact path', pathSuffix: '.fact-set.json' }), __fact_set_artifact_id: outline.factSetArtifactId }) }, exec)
    },
