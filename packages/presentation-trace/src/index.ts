...
import { defineArtifactProjection, type PaimindArtifactProjectionV1 } from '@paimind/contracts'
...
export interface PaimindPresentationTraceDocument {
  readonly schemaVersion: typeof TRACE_SCHEMA_V2
  ...
  private refresh(): void {
    ...
    if (sessionId !== undefined && value !== undefined) {
      try {
        ...
        traces = Object.freeze(projection.traces
          .filter(trace => trace.sessionId === sessionId)
          .map(trace => Object.freeze({ id: `projection:${trace.traceId}`, traceId: trace.traceId, document: trace.document })))
      } catch {
...
function normalizeTechnical(value: unknown, label: string): PaimindTraceTechnical | undefined {
  ...
  return Object.freeze({
    ...
    ...(value.filters === undefined ? {} : { filters: textList(value.filters, `${label}.filters`) }),
    ...(value.joinKeys === undefined ? {} : { joinKeys: textList(value.joinKeys, `${label}.joinKeys`) }),
...
function normalizeFact(value: unknown, label: string, sourceIds: ReadonlySet<string>): PaimindTraceFact {
  ...
  const dimensions = value.dimensions === undefined ? [] : (() => {
    ...
    const rows = value.dimensions.map((entry, index) => {
      ...
      return Object.freeze({ key: recordId(entry.key, `${label}.dimensions[${index}].key`), label: valueText(entry.label, `${label}.dimensions[${index}].label`), value: valueText(entry.value, `${label}.dimensions[${index}].value`) })
    })
    unique(rows.map(row => row.key), `${label}.dimension keys`)
    return rows
  })()
  const measures = value.measures === undefined ? [] : (() => {
    if (!Array.isArray(value.measures) || value.measures.length === 0) throw new Error(`${label}.measures must be a non-empty array`)
    const rows = value.measures.map((entry, index) => {
      ...
      return Object.freeze({ key: recordId(entry.key, `${label}.measures[${index}].key`), label: valueText(entry.label, `${label}.measures[${index}].label`), value: entry.value, displayValue: valueText(entry.displayValue, `${label}.measures[${index}].displayValue`) })
    })
  ...
  return Object.freeze({
    ...
    business: Object.freeze({
      ...
      scope: Object.freeze({ period: valueText(value.business.scope.period, `${label}.business.scope.period`), filters: textList(value.business.scope.filters, `${label}.business.scope.filters`) }),
    }),
...
function normalizeV2(value: UnknownRecord, sources: readonly PaimindTraceSourceRecord[]): PaimindPresentationTraceDocument {
  ...
  return Object.freeze({ schemaVersion: TRACE_SCHEMA_V2, reviewStatus: value.reviewStatus as PaimindTraceReviewStatus, sources, slides: Object.freeze(slides) })
}
