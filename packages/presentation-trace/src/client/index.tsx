...
import {
  markHarnessClientStyle, contributePaimindExtension, type HarnessSessionService, type PaimindClientContext } from '@paimind/harness-compat'
...
function installStyle(): () => void {
  ...
  const style = document.createElement('style'); style.id = STYLE_ID; style.dataset.paimindPlugin = '@paimind/presentation-trace'; markHarnessClientStyle(style, '@paimind/presentation-trace'); style.textContent = `${PAIMIND_UI_FOUNDATION_CSS}\n${STYLE}`; document.head.append(style)
  return () => { style.remove() }
}
...
export function PresentationTracePanel(props: { readonly service: PaimindPresentationTraceService; readonly bento: PaimindBentoPreviewService; readonly scope: PaimindSidebarTabScope }): React.JSX.Element {
  ...
  if (snapshot.selection === null) return <section data-paimind-ui-scope="presentation-trace" data-paimind-trace aria-label={zh ? '演示追溯' : 'Presentation Trace'}><div data-paimind-trace-empty>{zh ? '请从 PAIMind 产物中选择“追溯”。' : 'Choose Trace from a PAIMind artifact.'}</div></section>
