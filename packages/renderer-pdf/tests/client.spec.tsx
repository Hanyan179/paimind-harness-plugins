...
describe('FP06 PAIMind PDF renderer', () => {
  ...
  it('loads bytes locally and exposes paged navigation, zoom and boundary feedback', async () => {
    ...
    await waitFor(() => { expect(screen.getByRole('status', { name: '页码 / Page' })).toHaveTextContent('1 / 3') })
    const previous = screen.getByRole('button', { name: '上一页 / Previous page' })
    const next = screen.getByRole('button', { name: '下一页 / Next page' })
    expect(previous).toBeDisabled()
    expect(next).toBeEnabled()
    fireEvent.click(next)
    expect(screen.getByRole('status', { name: '页码 / Page' })).toHaveTextContent('2 / 3')
    fireEvent.click(next)
    expect(screen.getByRole('status', { name: '页码 / Page' })).toHaveTextContent('3 / 3')
    expect(next).toBeDisabled()
    fireEvent.click(screen.getByRole('button', { name: '放大 / Zoom in' }))
    expect(screen.getByRole('status', { name: '缩放 / Zoom' })).toHaveTextContent('125%')
    const viewport = screen.getByLabelText('PDF page viewport')
