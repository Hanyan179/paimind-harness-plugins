...
// ===========================================================================
const titleInput = el("input", { class: "title-input", value: "未命名表格", "aria-label": "表格标题" });
const statusDot = el("span", { class: "dot saved" });
const statusText = el("span", {}, "已保存");
const peersEl = el("div", { class: "peers" });
const topbar = el("div", { class: "topbar" }, [
  el("div", { class: "title-wrap" }, [titleInput]),
  el("div", { class: "spacer" }),
  el("div", { class: "status", title: "保存状态" }, [statusDot, statusText]),
]);
...
const sumBtn = iconBtn("sigma", "自动求和", () => autoSum());
const fxBtn = iconBtn(null, "Insert function", (e) => openFunctionMenu(e), "ƒx");

const currencyBtn = iconBtn("currency", "货币格式", () => setFmtOnSelection((f) => { f.nf = "currency"; }));
const percentBtn = iconBtn("percent", "百分比格式", () => setFmtOnSelection((f) => { f.nf = "percent"; }));
...
const strikeBtn = iconBtn("strike", "删除线", () => toggleFmt("s"));
...
const textColorBtn = colorBtn("textcolor", "文字颜色", "c", "#1d1d20");
const fillColorBtn = colorBtn("fill", "填充颜色", "bg", "#fff3a3");
...
alignBtns.l = segBtn("alignLeft", "左对齐", "l");
alignBtns.c = segBtn("alignCenter", "居中", "c");
alignBtns.r = segBtn("alignRight", "右对齐", "r");
const alignSegment = el("div", { class: "segment" }, [alignBtns.l, alignBtns.c, alignBtns.r]);

const wrapBtn = iconBtn("wrap", "自动换行", () => toggleFmt("wrap"));

const insRowBtn = iconBtn("insRow", "在上方插入行", () => insertRows(selRange().r1, 1));
const insColBtn = iconBtn("insCol", "在左侧插入列", () => insertCols(selRange().c1, 1));
const delRowBtn = iconBtn("trash", "删除选中行", () => deleteRows());
const sortAscBtn = iconBtn("sortAsc", "升序排序", () => sortSelection(true));
const sortDescBtn = iconBtn("sortDesc", "降序排序", () => sortSelection(false));
const clearBtn = iconBtn("clear", "清除格式", () => clearFormatting());
...
// --- Formula bar ---
const nameBox = el("input", { class: "namebox", "aria-label":"单元格地址", value: "A1", spellcheck: "false" });
const formulaInput = el("input", { class: "finput", "aria-label":"单元格值或公式", spellcheck: "false", placeholder: "" });
...
async function doSave() {
  ...
  if (!pendingCellOps.size && !pendingStructure && !pendingReplacements.size) { setStatus("saved", "已保存"); return; }
  ...
  try {
    ...
    if (result.replacedCells) for (const [sid, cells] of Object.entries(result.replacedCells)) model.cells[sid] = cells;
    ...
    saveInFlight = false;
    if (!saveFailed && (pendingCellOps.size || pendingStructure || pendingReplacements.size)) scheduleSave(40);
  }
}
...
gridScroll.addEventListener("keydown", (e) => {
  ...
  switch (e.key) {
    ...
    case "删除": case "Backspace": e.preventDefault(); deleteSelectionContents(); break;
...
// Title
// ===========================================================================
titleInput.addEventListener("input", () => { model.title = titleInput.value.trim() || "未命名表格"; queueStructure(); });
...
function openFunctionMenu(e) {
  const groups = {
    ...
    "文本": ["CONCAT", "TEXTJOIN", "LEFT", "RIGHT", "MID", "LEN", "UPPER", "LOWER", "TRIM", "SUBSTITUTE", "TEXT"],
...
gridTable.addEventListener("contextmenu", (e) => {
  ...
  item("剪切", "Ctrl+X", () => { copySelection(); deleteSelectionContents(); });
  item("复制", "Ctrl+C", () => copySelection());
  item("粘贴", "Ctrl+V", async () => { try { const t = await navigator.clipboard.readText(); pasteText(t); } catch (e) {} });
  sep();
  item("在上方插入行", "", () => insertRows(selRange().r1, 1));
  item("Insert row below", "", () => insertRows(selRange().r2 + 1, 1));
  item("在左侧插入列", "", () => insertCols(selRange().c1, 1));
  item("Insert column right", "", () => insertCols(selRange().c2 + 1, 1));
  sep();
  item("删除选中行", "", () => deleteRows(), true);
...
function renderTabs() {
  ...
  const add = el("button", { type:"button", class: "tab-add", title: "添加工作表", html: icon(ICONS.plus) });
...
function sheetTabMenu(id, e) {
  ...
  if (model.sheetOrder.length > 1) { menu.appendChild(el("div", { class: "ctx-sep" })); item("删除", () => deleteSheet(id), true); }
  showCtx(menu, e.clientX, e.clientY);
}
...
function promptInline(message, def = "") {
  return new Promise((resolve) => {
    ...
    const ok = el("button", { class: "primary" }, "确定");
...
function applyRemoteOperation(event) {
  ...
  setTimeout(() => { if (!saveInFlight && !pendingCellOps.size) setStatus("saved", "已保存"); }, 900);
}
