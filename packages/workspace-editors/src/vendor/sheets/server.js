// Adapted from Cloudflare OS b810d09f5358eeb4aeeef2acdde45aff54405277; see PROVENANCE.json.
class DurableObject {}
  ...
  async applyOperationLocked(operation) {
    ...
    for (const [sheetId, ops] of bySheet) {
      ...
      for (const op of ops) {
        ...
        if (isDelete) {
          if (!cur) { if (base !== 0) conflicts.push({ sheetId, ref, cell: null }); continue; }
          if (cur.version !== base) { conflicts.push({ sheetId, ref, cell: cur }); continue; }
          delete cells[ref];
          deletes.push({ sheetId, ref });
          dirty = true;
        } else {
          if ((cur?.version || 0) !== base) { conflicts.push({ sheetId, ref, cell: cur || null }); continue; }
          const next = {
            value: op.value == null ? "" : String(op.value).slice(0, 8192),
            fmt: sanitizeFmt(op.fmt),
            version: (cur?.version ?? meta.revision) + 1,
          };
          cells[ref] = next;
    ...
    // Structural changes invalidate cell versions, including cells moved to a
    // different row/column. A stale local edit must not overwrite a new cell at
    // the same address merely because its old version number was reused.
    if (operation.structure || operation.sheetReplacements?.length) {
      for (const id of meta.sheetOrder) {
        const cells = await this.loadCells(id);
        for (const cell of Object.values(cells)) cell.version = meta.revision + 1;
        await this.ctx.storage.put("cells:" + id, cells);
      }
    }
    meta.revision += 1;
    ...
    const event = {
      ...
      upserts,
      deletes,
      replacedSheets: operation.structure || operation.sheetReplacements?.length ? meta.sheetOrder.slice() : [],
      lastModified: meta.lastModified,
    };
