...
const COMPONENTS = {
  ...
  logo: {
    name: "Logo",
    defaultBlock: () => ({
      x: 36, y: 56, props: { variant: "light", scale: 1, text: "工作区",
        accentDot: true },
    }),
    fields: [
    ...
    render(props) {
      ...
      wrap.appendChild(el("div", {
        text: props.text != null ? props.text : "工作区",
        style: {
          color: textColor,
          fontFamily: FONT,
  ...
  title: {
    ...
    resizableW: true,
    fields: [
      { key: "text", label: "文本", type: "multiline" },
  ...
  text: {
    ...
    defaultBlock: () => ({
      x: 100, y: 200, w: 400,
      props: { text: "文本", fontSize: 19, color: "#000000", weight: 400,
...
const COMPONENT_CATEGORIES = [
  { name: "文本",       types: ["title", "subtitle", "text", "bulletList", "sectionLabel"] },
  { name: "容器", types: ["card", "box"] },
  { name: "媒体",      types: ["image", "svg"] },
  { name: "装饰", types: ["tonePill", "divider", "shape"] },
  { name: "图示",    types: ["arrow"] },
];
...
function makeContentSlide() {
  return {
    ...
    blocks: [
      { type: "sectionLabel", x: 36, y: 35, props: { text: "示例章节" } },
      { type: "title", x: 35, y: 76, w: 984,
        props: { text: "示例标题：写出本页的主要结论",
      ...
      { type: "text", x: 36, y: 204, w: 760,
        props: { text: "示例内容：在此补充支持结论的事实与依据，并为图表或素材保留空间。",
...
function renderAdvancedDisclosure(slide, block, def, advancedFields) {
  ...
  toggle.appendChild(el("span", { text: "更多属性" }));
...
function mountShell() {
  ...
  style.textContent = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    #leftPanel [aria-label*="删除"], #rightPanel [aria-label*="删除"] {
  ...
  const caret = el("button", {
    title: "选择页面布局", "aria-label": "选择页面布局",
    onclick: (e) => {
      e.stopPropagation();
  ...
  const prev = iconBtn({ icon: ICONS.chevronLeft, title: "上一页 (←)",
    onClick: () => go(currentIndex - 1) });
  const next = iconBtn({ icon: ICONS.chevronRight, title: "下一页 (→)",
    onClick: () => go(currentIndex + 1) });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    else if ((e.key === "删除" || e.key === "Backspace") &&
...
function renderSlideList() {
  ...
  for (const i of indices) {
    ...
    const dupBtn = thumbActionBtn(ICONS.copy, "复制页面", (e) => {
      e.stopPropagation(); duplicateSlideById(s.id);
    });
    const delBtn = thumbActionBtn(ICONS.trash, "删除页面", (e) => {
