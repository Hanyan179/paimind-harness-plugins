...
const titleInput = el("input", { class: "title-input", value: "未命名文档", "aria-label": "文档标题" });
...
const styleSel = customSelect({
  className: "style-sel", title: "段落样式",
  options: [
    { value: "P", label: "正文" },
...
const strikeBtn = cmdBtn("strike", "删除线", "strikeThrough");
...
const textColorBtn = colorBtn("textcolor", "文字颜色", "foreColor", "#1d1d20");
const highlightBtn = colorBtn("highlight", "高亮颜色", "hiliteColor", "#fff3a3");
...
alignBtns.left = segBtn("alignLeft", "左对齐", "justifyLeft");
alignBtns.center = segBtn("alignCenter", "居中", "justifyCenter");
alignBtns.right = segBtn("alignRight", "右对齐", "justifyRight");
alignBtns.justify = segBtn("alignJustify", "两端对齐", "justifyFull");
const alignSegment = el("div", { class: "segment" }, [alignBtns.left, alignBtns.center, alignBtns.right, alignBtns.justify]);

const ulBtn = cmdBtn("ul", "项目符号列表", "insertUnorderedList");
const olBtn = cmdBtn("ol", "编号列表", "insertOrderedList");
...
const clearBtn = iconBtn("clear", "清除格式", () => {
...
async function doSave() {
  ...
  try {
    ...
    const stillDirty = latest.some((b) => acknowledged.get(b.id)?.html !== b.html) ||
      ...
      (titleInput.value.trim() || "未命名文档") !== acknowledgedTitle;
    if (!saveFailed && (saveAgain || stillDirty)) {
      saveAgain = false;
      scheduleSave(40);
    }
  }
...
function applyRemoteOperation(event) {
  ...
  setTimeout(() => { if (!saveInFlight && !saveTimer) setStatus("saved", "已保存"); }, 900);
}
