...
export class Gadget extends DurableObject {
  ...
  async setDocumentLocked({ blocks, title, senderId }) {
    ...
    const document = {
      ...
      blocks: cleanBlocks.map((block) => ({
        id: block.id,
        html: block.html,
        version: (previousById.get(block.id)?.version ?? previous?.revision ?? 0) + 1,
      })),
  ...
  async applyOperationLocked(operation) {
    ...
    for (const incoming of sanitizeBlocks(operation.upserts || [])) {
      ...
      if (!current && expected !== 0) { conflicts.push({ id: incoming.id, deleted: true }); continue; }
      const next = { id: incoming.id, html: incoming.html, version: (current?.version ?? doc.revision) + 1 };
