...
const SUPPORT_PACKAGE_NAMES: Readonly<Record<string, readonly [string, string]>> = {
  '@paimind/agent-builder': ['智能体创建服务', 'Agent creation service'],
  '@paimind/workspace-project': ['工作区连接', 'Workspace connection'],
  '@paimind/better-sidebar-adapter': ['侧边文件与预览', 'Sidebar files and previews'],
  '@paimind/artifact-runtime': ['文件生成服务', 'File generation service'],
  '@paimind/category-analysis-adapter': ['类目分析', 'Category analysis'],
  '@paimind/fact-layer': ['数据依据', 'Data evidence'],
  '@paimind/walmart-proposal-adapter': ['沃尔玛采购提案', 'Walmart buyer proposals'],
  '@paimind/scheduler-adapter-harness': ['智能任务执行', 'AI task execution'],
  '@paimind/scheduler-adapter-http': ['外部服务调用', 'External service actions'],
  '@paimind/platform-api': ['系统集成接口', 'Integration API'],
  '@paimind/scheduler-adapter-feishu-bot': ['飞书消息交付', 'Feishu delivery'],
}
      ...
      <section data-paimind-pack-extensions aria-label={zh ? '包内扩展' : 'Extensions in this pack'}>
        ...
        {selectedPack.packageNames.map(packageName => {
          ...
          return <div key={packageName} data-paimind-pack-extension-row>
            <div><strong>{descriptor === undefined ? (SUPPORT_PACKAGE_NAMES[packageName]?.[zh ? 0 : 1] ?? packageName.replace('@paimind/', '')) : (zh ? descriptor.nameZh : descriptor.nameEn)}</strong><details data-paimind-extension-technical><summary>{zh ? '技术信息' : 'Technical details'}</summary><code>{packageName}</code></details></div>
