...
Optional type-only peers: `@hansen/agent-builder` and `@hansen/mcp-center`. Their public remote services are discovered when the panel opens and every five seconds while open; absence or errors are isolated per provider. No private repository paths or provider-specific logic are used.
