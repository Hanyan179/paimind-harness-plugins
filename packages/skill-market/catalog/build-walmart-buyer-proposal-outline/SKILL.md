...
2. Call `build_walmart_buyer_proposal_outline` with exactly those two Artifact IDs and a Workspace-relative `.outline.json` output path. Pass the user-confirmed visual choice as `style_preset` (`strategy-consulting`, `paramont-signature`, or `playful-storybook`); omission uses the legacy `wmt-retail` default. Verify the returned `design.stylePreset` matches before rendering.
