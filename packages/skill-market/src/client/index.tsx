...
function businessSkillRows(catalog: readonly SkillCatalogItem[], installed: readonly SkillInstallRecord[]) {
  ...
  return [...new Set([...available.keys(), ...local.keys()])].map(name => {
    ...
    return {
      ...
      displayName: record?.displayName ?? recommendation?.displayNameZh,
  ...
  const rows = useMemo(() => {
    ...
    return viewRows
      ...
      .filter(row => normalizedQuery === '' || [
        row.item.name, row.item.description, row.displayName ?? '', row.installed?.whenToUse ?? '',
  ...
  return <section ref={contentRoot} data-paimind-skill-market aria-label={zh ? '技能中心' : 'Skill Center'}>
    ...
    {editor !== null ? <section data-paimind-skill-editor aria-label={zh ? '技能文件夹编辑器' : 'Skill folder editor'}>
      ...
      <div role="tablist" aria-label={zh ? '技能中心主视图' : 'Skill Center views'} data-paimind-skill-primary-tabs>
        <button role="tab" type="button" aria-label={zh ? '全部技能' : 'All Skills'} aria-selected={tab === 'all'} tabIndex={tab === 'all' ? 0 : -1} onKeyDown={handlePaimindTabKey} onClick={activateAll}>{zh ? '全部技能' : 'All Skills'}<span>{catalogRows.length}{catalog.status !== 'ready' || installedStatus !== 'ready' ? '+' : ''}</span></button>
        ...
        <button role="tab" type="button" aria-label={zh ? '已安装' : 'Installed'} aria-selected={tab === 'installed'} tabIndex={tab === 'installed' ? 0 : -1} onKeyDown={handlePaimindTabKey} onClick={activateInstalled}>{zh ? '已安装' : 'Installed'}<span>{installedStatus === 'ready' ? installed.length : '—'}</span></button>
      </div>
      ...
      <section data-paimind-skill-catalog aria-label={zh ? '技能目录' : 'Skill catalog'}>
        ...
        {tab === 'all' ? <div data-paimind-skill-grid>
          <section data-paimind-skill-panel aria-label={zh ? '技能目录列表' : 'Catalog Skill list'}>{rows.length === 0 ? catalog.status !== 'ready' || installedStatus !== 'ready' ? null : <div data-paimind-skill-state><p>{zh ? '没有匹配的技能。调整筛选或清除搜索即可继续。' : 'No matching Skills. Adjust filters or clear search to continue.'}</p>{filtersActive && <button type="button" data-paimind-skill-button onClick={resetFilters}>{zh ? '清除筛选' : 'Clear filters'}</button>}</div> : <ul data-paimind-skill-list>{visibleRows.map(row => <li key={row.item.name} data-paimind-skill-row data-selected={detail?.name === row.item.name}><button type="button" data-paimind-skill-select onClick={() => { setSelected(row.item.name); setMobileDetailOpen(window.matchMedia?.('(max-width:760px)').matches === true) }} aria-label={`${zh ? '查看' : 'View'}: ${zh ? row.displayName ?? row.item.name : row.item.name}`}><span data-paimind-skill-row-icon><PaimindSkillIcon size={20} /></span><span data-paimind-skill-row-copy><span data-paimind-skill-name>{zh ? row.displayName ?? row.item.name : row.item.name}</span><span data-paimind-skill-description>{row.item.description}</span></span><span data-paimind-skill-row-badges><span data-paimind-skill-category>{categoryLabel(row.metadata.category, zh)}</span><span data-paimind-skill-policy>{row.installed !== undefined ? (zh ? '已安装' : 'Installed') : installedStatus !== 'ready' ? (zh ? '安装状态未知' : 'Install status unknown') : (zh ? '可安装' : 'Available')}</span></span></button></li>)}{visibleRows.length < rows.length && <li data-paimind-skill-load-more><button type="button" data-paimind-skill-button onClick={() => { setVisibleMarketCount(value => value + MARKET_PAGE_SIZE) }}>{zh ? `加载更多（剩余 ${rows.length - visibleRows.length} 个）` : `Load more (${rows.length - visibleRows.length} remaining)`}</button></li>}</ul>}</section>
          <aside data-paimind-skill-panel data-paimind-skill-detail role={mobileDetailOpen ? 'dialog' : undefined} aria-modal={mobileDetailOpen || undefined} data-mobile-open={mobileDetailOpen} onKeyDown={event => { if (mobileDetailOpen) handleDialogKey(event, () => { setMobileDetailOpen(false) }) }} aria-label={zh ? '技能详情' : 'Skill details'}>
            <button type="button" ref={mobileDetailClose} data-paimind-skill-mobile-close aria-label={zh ? '返回技能列表' : 'Back to Skill list'} onClick={() => { setMobileDetailOpen(false) }}><PaimindCloseIcon size={17} /></button>
            {detail === undefined ? <p>{zh ? '选择一个技能查看详情。' : 'Select a Skill.'}</p> : <>
              <span data-paimind-skill-detail-icon><PaimindSkillIcon size={25} /></span>
              <div><h2>{zh ? detailRow?.displayName ?? detail.name : detail.name}</h2><p data-paimind-skill-detail-subtitle>{detail.description}</p></div>
