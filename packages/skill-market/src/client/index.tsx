import { PAIMIND_UI_FOUNDATION_CSS, handlePaimindTabKey } from '@paimind/ui-foundation'
import {
  Component,
  useEffect,
  useLayoutEffect,
...
const CATEGORY_LABELS: Readonly<Record<SkillProductCategory, Readonly<{ zh: string; en: string }>>> = Object.freeze({
  ...
  'agent-tools': Object.freeze({ zh: '智能体与自动化', en: 'Agents & automation' }),
})

function systemSkillPresentation(skill: Readonly<PaimindSystemSkillReference>, zh: boolean): { name: string; description: string } {
  const owned: Record<string, readonly [string, string, string]> = {
    'paimind-agent-authoring': ['@paimind/agent-builder', '创建智能体', '根据你的工作目标准备智能体草稿，确认后再保存。'],
    'paimind-skill-authoring': ['@paimind/skill-market', '创建技能', '把工作方法整理为可复用的技能，确认后再保存。'],
    'paimind-skill-installation': ['@paimind/skill-market', '安装技能', '检查并安装你提供的技能文件或链接。'],
    genui: ['@deepseek-ai/dsh-tool-genui', '交互内容', '在对话中生成可操作的内容，帮助查看和理解结果。'],
  }
  const value = owned[skill.name]
  return zh && value !== undefined && value[0] === skill.sourcePluginId
    ? { name: value[1], description: value[2] }
...
/** Unsaved UI state belongs to this client service instance, never browser storage. */
const skillEditorDrafts = new WeakMap<SkillInstallerRemoteNamespace, SkillEditorDraft>()
const skillDraftGuards = new WeakMap<SkillInstallerRemoteNamespace, () => void>()
function retainSkillEditorDraft(installer: SkillInstallerRemoteNamespace, editor: SkillEditorDraft | null): void {
  skillDraftGuards.get(installer)?.()
  skillDraftGuards.delete(installer)
  if (editor === null) { skillEditorDrafts.delete(installer); return }
  skillEditorDrafts.set(installer, editor)
  const dirty = editor.files.some(file => file.state !== 'clean') || editor.directories.some(folder => folder.state === 'new')
  if (!dirty) return
  const beforeUnload = (event: BeforeUnloadEvent): void => { event.preventDefault(); event.returnValue = '' }
  window.addEventListener('beforeunload', beforeUnload)
  skillDraftGuards.set(installer, () => { window.removeEventListener('beforeunload', beforeUnload) })
}
...
function initialSkillPackage(): SkillEditorDraft {
  const source = `---\nname: my-skill\ndescription: "说明这个技能能帮助完成什么工作，以及何时使用。"\n---\n\n# 我的技能\n\n在这里编写工作流程、输入、输出、示例与边界。\n`
...
function handleDialogKey(event: ReactKeyboardEvent<HTMLElement>, close: () => void): void {
  ...
  const focusable = [...event.currentTarget.querySelectorAll<HTMLElement>('button:not(:disabled), [href], input:not(:disabled), select:not(:disabled), textarea:not(:disabled), summary, [tabindex]:not([tabindex="-1"])')]
...
export function SkillMarketSection(props: SkillMarketSectionProps): React.JSX.Element {
  ...
  const [editor, setEditor] = useState<SkillEditorDraft | null>(() => skillEditorDrafts.get(props.installer) ?? null)
  ...
  const mobileDetailClose = useRef<HTMLButtonElement>(null)
  const contentRoot = useRef<HTMLElement>(null)
  const editingSkill = editor !== null

  useEffect(() => {
    const surface = contentRoot.current?.closest<HTMLElement>("[data-paimind-product-surface='skill-center']")
    if (surface !== null && surface !== undefined) surface.scrollTop = 0
  }, [editingSkill, tab])

  useEffect(() => {
    if (!mobileDetailOpen || !window.matchMedia?.('(max-width:760px)').matches) return
    const trigger = document.activeElement instanceof HTMLElement ? document.activeElement : null
    const surface = mobileDetailClose.current?.closest<HTMLElement>("[data-paimind-product-surface='skill-center']")
    const scrollTop = surface?.scrollTop ?? 0
    if (surface !== null && surface !== undefined) surface.scrollTop = 0
    mobileDetailClose.current?.focus({ preventScroll: true })
    return () => {
      if (surface?.isConnected) surface.scrollTop = scrollTop
      if (trigger?.isConnected) trigger.focus({ preventScroll: true })
    }
  }, [mobileDetailOpen])

  useEffect(() => () => { uploadAbort.current?.abort() }, [])
  useEffect(() => { retainSkillEditorDraft(props.installer, editor) }, [editor, props.installer])
  ...
  const installedRows = useMemo(() => installed.filter(item => query.trim() === '' || `${item.name} ${item.displayName ?? ''} ${item.description} ${item.whenToUse ?? ''}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())), [installed, query])
  const systemRows = useMemo(() => systemSkills.status !== 'ready' ? [] : systemSkills.items.filter(item => query.trim() === '' || `${item.name} ${systemSkillPresentation(item, zh).name} ${systemSkillPresentation(item, zh).description} ${item.description} ${item.whenToUse ?? ''} ${item.sourcePluginId}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())), [query, systemSkills, zh])
  ...
  const inspectFolder = async (files: FileList): Promise<void> => {
    ...
    if (rows.length > 50_000) { setError(zh ? '技能文件夹超过 50000 个文件。' : 'The Skill folder exceeds 50,000 files.'); return }
    ...
    try {
      ...
      if (paths.some(path => path.split('/')[0] !== root)) throw new Error(zh ? '请选择一个完整的技能文件夹。' : 'Choose one complete Skill folder.')
      const total = rows.reduce((sum, file) => sum + file.size, 0)
      if (total > 200 * 1024 * 1024) throw new Error(zh ? '技能文件夹超过 200 MB。' : 'The Skill folder exceeds 200 MB.')
  ...
  const confirmInstall = async (): Promise<void> => {
    ...
    try {
      ...
      setNotice(result.operation === 'updated' ? (zh ? `${preview.displayName ?? preview.name} 已更新，旧版本已保留备份。` : `${preview.displayName ?? preview.name} updated atomically; the prior version is restored on failure.`) : (zh ? `${preview.displayName ?? preview.name} 已保存到技能中心。` : `${preview.displayName ?? preview.name} saved to the Skill Center.`))
    } catch (reason) { setError(messageOf(reason)) } finally { setBusy(false) }
  }
  const uninstall = async (): Promise<void> => {
    ...
    try {
      ...
      setNotice(removal.recoverable ? (zh ? `${item.displayName ?? item.name} 已卸载，原内容已移入可恢复备份。` : `${item.displayName ?? item.name} uninstalled; its prior contents are in a recoverable backup.`) : (zh ? `${item.displayName ?? item.name} 已卸载。` : `${item.displayName ?? item.name} uninstalled.`))
    } catch (reason) { setError(messageOf(reason)) } finally { setBusy(false) }
  }

  const replaceBusinessPolicy = async (name: string, field: 'enabled' | 'direct', checked: boolean): Promise<void> => {
    const label = installed.find(item => item.name === name)?.displayName ?? name
    ...
    try {
      ...
      setNotice(field === 'enabled'
        ? checked ? (zh ? `${label} 已启用，可以选择用在哪里。` : `${label} is now available for use.`) : (zh ? `${label} 已停用，普通对话也不再默认使用。` : `${label} is disabled and removed from direct-chat defaults.`)
        : checked ? (zh ? `普通对话将默认使用${label}。` : `${label} is now used by default in direct chats.`) : (zh ? `普通对话不再默认使用${label}。` : `${label} is no longer a direct-chat default.`))
    } catch (reason) { setError(messageOf(reason)) } finally { setBusy(false) }
  }
  ...
  const replaceSystemPolicy = async (skill: Readonly<PaimindSystemSkillReference>, checked: boolean): Promise<void> => {
    ...
    try {
      ...
      setNotice(checked
        ? (zh ? `${systemSkillPresentation(skill, zh).name}已启用。` : `${skill.name} enabled.`)
        : (zh ? `${systemSkillPresentation(skill, zh).name}已停用。` : `${skill.name} disabled.`))
  ...
  const replaceSessionPolicy = async (name: string, checked: boolean): Promise<void> => {
    ...
    if (sessionId === undefined || sessionSelection.status !== 'ready' || props.installer.replaceSessionBusinessSkillSelection === undefined) {
      setError(zh ? '当前对话临时范围尚未就绪，未更改任何技能。' : 'The current conversation scope is not ready; no Skill was changed.')
      return
    }
    ...
    if (checked && !alreadySelected && !userEnabled) {
      setError(zh ? '请先启用这个业务技能，再添加到当前对话。' : 'Enable this Business Skill before adding it to the current conversation.')
      return
    }
    ...
    try {
      ...
      setNotice(checked
        ? (zh ? `当前对话已添加${label}；服务重启后需重新选择。` : `${label} was added to this conversation and clears when the service restarts.`)
        : (zh ? `当前对话不再使用${label}。` : `${label} was removed from this conversation.`))
    } catch (reason) {
  ...
  const editSkill = async (skillId: string): Promise<void> => {
    ...
    try {
      ...
      if (!source.managed) throw new Error(zh ? '这个技能由外部来源管理，不能在技能中心直接修改。' : 'This Skill is externally managed and cannot be edited here.')
  ...
  const saveEditor = async (): Promise<void> => {
    ...
    try {
      ...
      setNotice(result.operation === 'installed'
        ? (zh ? `${result.record.displayName ?? result.record.name} 已创建并保存。` : `${result.record.displayName ?? result.record.name} created and saved.`)
        : (zh ? `${result.record.displayName ?? result.record.name} 已保存新版本。` : `${result.record.displayName ?? result.record.name} saved as a new version.`))
    } catch (reason) { setError(messageOf(reason)) } finally { setBusy(false) }
  }
  ...
  const sessionScopeCopy = sessionSelection.status === 'no-session'
    ...
    : sessionSelection.status === 'unavailable'
      ? (zh ? '当前版本未提供真实的对话临时选择接口。' : 'This version does not expose the real Session selection API.')
      : sessionSelection.status === 'loading'
        ...
        : sessionSelection.status === 'error'
          ...
          : props.installer.replaceSessionBusinessSkillSelection === undefined
            ? (zh ? '当前版本未提供真实的对话临时选择写入接口。' : 'This version does not expose the real Session selection write API.')
            : installedSessionSelected && !installedUserEnabled
            ? (zh ? '这个技能已停用，但仍可从当前对话移除；宿主重启后也会清空。' : 'This Skill is disabled but can still be removed here; it also clears when the host restarts.')
            : !installedUserEnabled
              ? (zh ? '请先启用这个业务技能；临时选择仅在当前对话生效。' : 'Enable this Business Skill first; temporary selection applies only to this Session.')
              : (zh ? '仅用于当前对话，服务重启后需重新选择。' : 'Runtime-ephemeral: only this Session is affected, and the selection clears when the host restarts.')
  ...
  const openAgentCenterForAttachment = (): void => {
    if (!isPaimindProductSurfaceAvailable('agent-center')) {
      setError(zh ? '智能体中心当前不可用，未添加任何技能。' : 'Agent Center is unavailable. No Skill was attached.')
      return
    }
  ...
  return <section ref={contentRoot} data-paimind-skill-market aria-label={zh ? '技能中心' : 'Skill Center'}>
    ...
    <div data-paimind-skill-feedback aria-live="polite" aria-atomic="true">
      {uploading && <div role="status" data-paimind-skill-notice><span>{zh ? '正在安全读取并检查本地技能…' : 'Securely reading and inspecting the local Skill…'}</span><button type="button" data-paimind-skill-inline-action onClick={() => { uploadAbort.current?.abort() }}>{zh ? '取消' : 'Cancel'}</button></div>}
    ...
    {editor !== null ? <section data-paimind-skill-editor aria-label={zh ? '技能文件夹编辑器' : 'Skill folder editor'}>
      <div data-paimind-skill-editor-head><div><p data-paimind-skill-eyebrow>{editor.authoringDraftId === undefined ? (editor.expectedDigest === undefined ? (zh ? '新建技能包' : 'New Skill package') : (zh ? '编辑已安装技能包' : 'Edit installed Skill package')) : (zh ? 'AI 已准备 · 尚未保存' : 'Prepared by AI · not saved')}</p><h2>{installed.find(item => item.skillId === editor.skillId)?.displayName ?? editor.skillId ?? (zh ? '未保存的技能' : 'Unsaved Skill folder')}</h2><p>{zh ? '在 SKILL.md 中编写技能说明，可添加参考资料、脚本和素材。保存时会一起保存整个文件夹。' : 'SKILL.md is the entry file. Scripts, references, assets, and other files belong to the same atomically saved package.'}</p></div><span data-paimind-skill-policy>{zh ? `${new Set([...editor.entries.filter(entry => entry.kind !== 'directory').map(entry => entry.path), ...editor.files.filter(file => file.state !== 'deleted').map(file => file.path)]).size} 个已加载文件` : `${new Set([...editor.entries.filter(entry => entry.kind !== 'directory').map(entry => entry.path), ...editor.files.filter(file => file.state !== 'deleted').map(file => file.path)]).size} loaded files`}</span></div>
      <div data-paimind-skill-package-editor>
        <aside data-paimind-skill-package-tree aria-label={zh ? '技能文件树' : 'Skill file tree'}>
        ...
        <section data-paimind-skill-package-workbench aria-label={zh ? '技能文件内容' : 'Skill file content'}>
      ...
      <div role="tablist" aria-label={zh ? '技能中心主视图' : 'Skill Center views'} data-paimind-skill-primary-tabs>
        <button role="tab" type="button" aria-label={zh ? '市场' : 'Market'} aria-selected={tab === 'market'} tabIndex={tab === 'market' ? 0 : -1} onKeyDown={handlePaimindTabKey} onClick={activateAll}>{zh ? '市场' : 'Market'}<span>{catalog.status === 'ready' ? catalog.items.length : 0}</span></button>
        <button role="tab" type="button" aria-label={zh ? '内置' : 'Built-in'} aria-selected={tab === 'builtin'} tabIndex={tab === 'builtin' ? 0 : -1} onKeyDown={handlePaimindTabKey} onClick={activateBuiltIn}>{zh ? '内置' : 'Built-in'}<span>{systemSkills.status === 'ready' ? systemSkills.items.length : 0}</span></button>
        <button role="tab" type="button" aria-label={zh ? '已安装' : 'Installed'} aria-selected={tab === 'installed'} tabIndex={tab === 'installed' ? 0 : -1} onKeyDown={handlePaimindTabKey} onClick={activateInstalled}>{zh ? '已安装' : 'Installed'}<span>{installed.length}</span></button>
      </div>
      ...
      <section data-paimind-skill-catalog aria-label={zh ? '技能目录' : 'Skill catalog'}>
        ...
        <div data-paimind-skill-filterbar>
          {tab === 'market' ? <div data-paimind-skill-filters>
            ...
            <select data-paimind-skill-select-filter aria-label={zh ? '按来源筛选' : 'Filter by source'} value={sourceFilter} onChange={event => { setSourceFilter(event.currentTarget.value) }}><option value="all">{zh ? '全部来源' : 'All sources'}</option>{sources.map(source => <option key={source} value={source}>{zh ? source === 'OpenAI Official · Adapted for Harness' ? 'OpenAI 官方' : source === 'PAIMind Internal · Adapted for Harness' ? 'PAIMind 提供' : source : source}</option>)}</select>
        ...
        {tab === 'market' ? <div data-paimind-skill-grid>
          <section data-paimind-skill-panel aria-label={zh ? '技能目录列表' : 'Catalog Skill list'}>{catalog.status === 'loading' ? <div data-paimind-skill-state aria-busy="true"><span data-paimind-skill-loading-dot />{zh ? '正在加载技能…' : 'Reading the live catalog…'}</div> : catalog.status === 'error' ? <div data-paimind-skill-state data-error="true" role="alert"><p>{catalog.error}</p><button type="button" data-paimind-skill-button onClick={() => { setRevision(value => value + 1) }}>{zh ? '重试' : 'Retry'}</button></div> : rows.length === 0 ? <div data-paimind-skill-state><p>{zh ? '没有匹配的技能。调整筛选或清除搜索即可继续。' : 'No matching Skills. Adjust filters or clear search to continue.'}</p><button type="button" data-paimind-skill-button onClick={resetFilters}>{zh ? '清除筛选' : 'Clear filters'}</button></div> : <ul data-paimind-skill-list>{visibleRows.map(row => <li key={row.item.id} data-paimind-skill-row data-selected={detail?.id === row.item.id}><button type="button" data-paimind-skill-select onClick={() => { setSelected(row.item.id); setMobileDetailOpen(window.matchMedia?.('(max-width:760px)').matches === true) }} aria-label={`${zh ? '查看' : 'View'}: ${zh ? row.item.displayNameZh ?? row.item.name : row.item.name}`}><span data-paimind-skill-row-icon><PaimindSkillIcon size={20} /></span><span data-paimind-skill-row-copy><span data-paimind-skill-name>{zh ? row.item.displayNameZh ?? row.item.name : row.item.name}</span><span data-paimind-skill-description>{row.item.description}</span></span><span data-paimind-skill-row-badges><span data-paimind-skill-category>{categoryLabel(row.metadata.category, zh)}</span><span data-paimind-skill-policy>{installedNames.has(row.item.name) ? (zh ? '已安装' : 'Installed') : (zh ? '可安装' : 'Available')}</span></span></button></li>)}{visibleRows.length < rows.length && <li data-paimind-skill-load-more><button type="button" data-paimind-skill-button onClick={() => { setVisibleMarketCount(value => value + MARKET_PAGE_SIZE) }}>{zh ? `加载更多（剩余 ${rows.length - visibleRows.length} 个）` : `Load more (${rows.length - visibleRows.length} remaining)`}</button></li>}</ul>}</section>
          <aside data-paimind-skill-panel data-paimind-skill-detail role={mobileDetailOpen ? 'dialog' : undefined} aria-modal={mobileDetailOpen || undefined} data-mobile-open={mobileDetailOpen} onKeyDown={event => { if (mobileDetailOpen) handleDialogKey(event, () => { setMobileDetailOpen(false) }) }} aria-label={zh ? '技能详情' : 'Skill details'}><button type="button" ref={mobileDetailClose} data-paimind-skill-mobile-close aria-label={zh ? '返回技能列表' : 'Back to Skill list'} onClick={() => { setMobileDetailOpen(false) }}><PaimindCloseIcon size={17} /></button>{detail === undefined ? <p>{zh ? '选择一个技能查看详情。' : 'Select a Skill.'}</p> : <><span data-paimind-skill-detail-icon><PaimindSkillIcon size={25} /></span><div><h2>{zh ? detail.displayNameZh ?? detail.name : detail.name}</h2><p data-paimind-skill-detail-subtitle>{detail.description}</p></div><div data-paimind-skill-statuses>{detailRow !== undefined && <span data-paimind-skill-category>{categoryLabel(detailRow.metadata.category, zh)}</span>}{detailInstall !== undefined && <span data-paimind-skill-policy>{zh ? '已安装' : 'Installed'}</span>}{detailInstall?.runtimeRequirements.length ? <span data-paimind-skill-policy data-warning="true">{zh ? '运行环境待确认' : 'Runtime setup to verify'}</span> : null}</div><details data-paimind-skill-disclosure><summary>{zh ? '来源与包信息' : 'Source and package details'}</summary><dl data-paimind-skill-meta><div data-paimind-skill-meta-row><dt>{zh ? '版本' : 'Version'}</dt><dd>{detail.version}</dd></div><div data-paimind-skill-meta-row><dt>{zh ? '来源' : 'Source'}</dt><dd>{detail.source}</dd></div><div data-paimind-skill-meta-row><dt>{zh ? '许可' : 'License'}</dt><dd>{detail.license}</dd></div></dl></details><div data-paimind-skill-actions>{detailInstall === undefined ? <button type="button" data-paimind-skill-button data-primary="true" disabled={busy} onClick={() => { void inspectRecommended(detail) }}><PaimindCheckIcon size={14} />{zh ? '检查并安装' : 'Review and install'}</button> : !detailCurrent ? <button type="button" data-paimind-skill-button data-primary="true" disabled={busy} onClick={() => { void inspectRecommended(detail) }}><PaimindCheckIcon size={14} />{zh ? '更新' : 'Update'}</button> : null}</div></>}</aside>
        </div> : tab === 'builtin' ? systemSkills.status === 'loading' ? <div data-paimind-skill-state aria-busy="true"><span data-paimind-skill-loading-dot />{zh ? '正在读取内置技能…' : 'Reading built-in Skills…'}</div> : systemSkills.status === 'error' ? <div data-paimind-skill-state data-error="true" role="alert"><p>{systemSkills.error}</p><button type="button" data-paimind-skill-button onClick={() => { setRevision(value => value + 1) }}>{zh ? '重试' : 'Retry'}</button></div> : systemSkills.status === 'unavailable' ? <div data-paimind-skill-state data-paimind-skill-contract-state><span data-paimind-skill-policy>{zh ? '未连接' : 'Not connected'}</span><h2>{zh ? '内置技能来源尚未接通' : 'Built-in Skill descriptors are not connected'}</h2><p>{zh ? '暂时无法读取内置技能，请稍后重新打开技能中心。' : 'This version does not expose verified System Skill descriptors. This view does not guess by name or expose UI-only switches.'}</p><small>{zh ? '安装并启用相关扩展后，对应技能会显示在这里。' : 'Built-in Skills appear here after Source Plugins expose their descriptor and lifecycle contract.'}</small></div> : systemRows.length === 0 ? <div data-paimind-skill-state><p>{zh ? '没有匹配的内置技能。' : 'No matching built-in Skills.'}</p>{query.trim() !== '' && <button type="button" data-paimind-skill-button onClick={resetFilters}>{zh ? '清除搜索' : 'Clear search'}</button>}</div> : <ul data-paimind-system-list>{systemRows.map(skill => {
          ...
          return <li key={skill.canonicalId} data-paimind-system-row><span data-paimind-skill-row-icon><PaimindSkillIcon size={20} /></span><span data-paimind-skill-row-copy><span data-paimind-skill-name>{systemSkillPresentation(skill, zh).name}</span><span data-paimind-skill-description>{systemSkillPresentation(skill, zh).description}</span></span>{mandatory ? <span data-paimind-skill-policy>{zh ? '必需 · 锁定' : 'Mandatory · locked'}</span> : <label data-paimind-system-control><span>{checked ? (zh ? '已开启' : 'On') : (zh ? '已关闭' : 'Off')}</span><input type="checkbox" role="switch" aria-label={zh ? `启用 ${systemSkillPresentation(skill, zh).name}` : `Enable ${skill.name}`} checked={checked} disabled={!policyConnected || busy} onChange={event => { void replaceSystemPolicy(skill, event.currentTarget.checked) }} /></label>}</li>
        })}</ul> : installedRows.length === 0 ? <div data-paimind-skill-state><p>{zh ? '还没有已安装的个人技能。可从市场安装，或导入本地 SKILL.md / ZIP。' : 'No personal Skills installed. Install from the market or import a local SKILL.md / ZIP.'}</p><div data-paimind-skill-actions><button type="button" data-paimind-skill-button data-primary="true" onClick={activateAll}>{zh ? '浏览市场' : 'Browse market'}</button><button type="button" data-paimind-skill-button onClick={chooseUpload}>{zh ? '本地导入' : 'Import local'}</button></div></div> : <div data-paimind-skill-grid>
          <section data-paimind-skill-panel aria-label={zh ? '已安装技能列表' : 'Installed Skill list'}><ul data-paimind-skill-list>{installedRows.map(item => <li key={item.skillId} data-paimind-skill-row data-selected={installedDetail?.skillId === item.skillId}><button type="button" data-paimind-skill-select onClick={() => { setSelectedInstalled(item.skillId); setMobileDetailOpen(window.matchMedia?.('(max-width:760px)').matches === true) }} aria-label={`${zh ? '查看已安装' : 'View installed'}: ${item.displayName ?? item.name}`}><span data-paimind-skill-row-icon><PaimindSkillIcon size={20} /></span><span data-paimind-skill-row-copy><span data-paimind-skill-name>{item.displayName ?? item.name}</span><span data-paimind-skill-description>{item.description}</span></span><span data-paimind-skill-policy>{item.managed ? (zh ? '可编辑' : 'Editable') : (zh ? '外部管理' : 'External')}</span></button></li>)}</ul></section>
          <aside data-paimind-skill-panel data-paimind-skill-detail role={mobileDetailOpen ? 'dialog' : undefined} aria-modal={mobileDetailOpen || undefined} data-mobile-open={mobileDetailOpen} onKeyDown={event => { if (mobileDetailOpen) handleDialogKey(event, () => { setMobileDetailOpen(false) }) }} aria-label={zh ? '已安装技能详情' : 'Installed Skill details'}>
            <button type="button" ref={mobileDetailClose} data-paimind-skill-mobile-close aria-label={zh ? '返回已安装列表' : 'Back to installed list'} onClick={() => { setMobileDetailOpen(false) }}><PaimindCloseIcon size={17} /></button>
            {installedDetail !== undefined && <>
              <span data-paimind-skill-detail-icon><PaimindSkillIcon size={25} /></span>
              <div><h2>{installedDetail.displayName ?? installedDetail.name}</h2><p data-paimind-skill-detail-subtitle>{installedDetail.description}</p></div>
    ...
    {uploadGuideOpen && <div data-paimind-skill-dialog-backdrop><section role="dialog" aria-modal="true" aria-labelledby="paimind-skill-import-guide-title" aria-describedby="paimind-skill-import-guide-copy" data-paimind-ui-scope="skill-market" data-paimind-skill-dialog data-paimind-skill-import-guide onKeyDown={event => { handleDialogKey(event, () => { setUploadGuideOpen(false) }) }}>
      <div data-paimind-skill-dialog-head><span data-paimind-skill-detail-icon><PaimindUploadIcon size={24} /></span><div><p data-paimind-skill-eyebrow>{zh ? '本地个人导入' : 'Local personal import'}</p><h2 id="paimind-skill-import-guide-title">{zh ? '选择一个可识别的技能包' : 'Choose a recognizable Skill package'}</h2></div></div>
      ...
      <div data-paimind-skill-import-formats><section><strong>SKILL.md</strong><p>{zh ? '适合只有一份说明文件的技能。文件必须包含 YAML Frontmatter（至少 name 与 description）。' : 'For a single-file Skill. It must include YAML frontmatter with at least name and description.'}</p></section><section><strong>ZIP</strong><p>{zh ? '适合包含脚本、参考资料或资源的技能。ZIP 根目录必须能找到 SKILL.md。' : 'For Skills with scripts, references, or assets. SKILL.md must be discoverable at the ZIP root.'}</p></section></div>
      <div data-paimind-skill-import-tree aria-label={zh ? '技能包结构示例' : 'Skill package structure example'}><code>my-skill/<br />├── SKILL.md <b>{zh ? '必需' : 'required'}</b><br />├── scripts/ <i>{zh ? '可选' : 'optional'}</i><br />├── references/ <i>{zh ? '可选' : 'optional'}</i><br />└── assets/ <i>{zh ? '可选' : 'optional'}</i></code></div>
      <ul data-paimind-skill-import-rules><li>{zh ? '不要包含密码、令牌或其他凭证。' : 'Do not include passwords, tokens, or other credentials.'}</li><li>{zh ? '安装后可在个人技能中查看，再决定用在哪里。' : 'This phase installs only to the current user’s personal Skill scope.'}</li><li>{zh ? '安装前会检查文件内容，确认后才会保存到技能中心。' : 'PAIMind reviews and installs; Harness remains responsible for discovery and execution.'}</li></ul>
    ...
    {preview !== null && <div data-paimind-skill-dialog-backdrop><section role="dialog" aria-modal="true" aria-labelledby="paimind-skill-install-title" data-paimind-ui-scope="skill-market" data-paimind-skill-dialog onKeyDown={event => { handleDialogKey(event, () => { if (!busy) setPreview(null) }) }}><div data-paimind-skill-dialog-head><span data-paimind-skill-detail-icon><PaimindSkillIcon size={24} /></span><div><p data-paimind-skill-eyebrow>{preview.operation === 'update' ? (zh ? '更新检查' : 'Update review') : (zh ? '安装检查' : 'Install review')}</p><h2 id="paimind-skill-install-title">{preview.operation === 'update' ? (zh ? `更新 ${preview.displayName ?? preview.name}` : `Update ${preview.displayName ?? preview.name}`) : (zh ? `安装 ${preview.displayName ?? preview.name}` : `Install ${preview.displayName ?? preview.name}`)}</h2></div></div><p>{preview.description}</p><dl data-paimind-skill-review-grid><div><dt>{zh ? '文件' : 'Files'}</dt><dd>{preview.fileCount}</dd></div><div><dt>{zh ? '解压大小' : 'Expanded'}</dt><dd>{new Intl.NumberFormat(locale).format(preview.expandedBytes)} B</dd></div><div><dt>{zh ? '操作' : 'Operation'}</dt><dd>{preview.operation === 'update' ? (zh ? '更新现有技能' : 'Atomic replace') : (zh ? '全新安装' : 'New install')}</dd></div></dl>{preview.operation === 'update' && <p data-paimind-skill-safe-copy>{zh ? '更新失败时会恢复旧版本；更新成功后也会保留旧版本备份。' : 'The Installer restores the previous version if update fails; after success, the prior version remains recoverable.'}</p>}{preview.runtimeRequirements.length > 0 && <p data-paimind-skill-warning><PaimindWarningIcon size={14} />{zh ? `运行环境待确认：${preview.runtimeRequirements.map(value => value === 'python' ? 'Python 依赖' : value === 'node' ? 'Node.js 依赖' : '系统依赖').join('、')}。安装完成不代表依赖已就绪。` : `Runtime setup to verify: ${preview.runtimeRequirements.join(', ')}. Installed does not mean runtime-ready.`}</p>}{preview.warnings.map(value => <p key={value} data-paimind-skill-warning><PaimindWarningIcon size={14} />{value}</p>)}<div data-paimind-skill-dialog-actions><button ref={previewPrimary} type="button" data-paimind-skill-button data-primary="true" disabled={busy} onClick={() => { void confirmInstall() }}><PaimindCheckIcon size={14} />{preview.operation === 'update' ? (zh ? '确认更新' : 'Confirm update') : (zh ? '确认安装' : 'Confirm install')}</button><button type="button" data-paimind-skill-button disabled={busy} onClick={() => { setPreview(null) }}>{zh ? '取消' : 'Cancel'}</button></div></section></div>}
    {uninstallTarget !== null && <div data-paimind-skill-dialog-backdrop><section role="dialog" aria-modal="true" aria-labelledby="paimind-skill-uninstall-title" data-paimind-ui-scope="skill-market" data-paimind-skill-dialog onKeyDown={event => { handleDialogKey(event, () => { if (!busy) setUninstallTarget(null) }) }}><div data-paimind-skill-dialog-head data-danger="true"><span data-paimind-skill-detail-icon><PaimindTrashIcon size={22} /></span><div><p data-paimind-skill-eyebrow>{zh ? '可恢复卸载' : 'Recoverable uninstall'}</p><h2 id="paimind-skill-uninstall-title">{zh ? `卸载 ${uninstallTarget.displayName ?? uninstallTarget.name}` : `Uninstall ${uninstallTarget.displayName ?? uninstallTarget.name}`}</h2></div></div><p>{zh ? '卸载后将无法在新任务中使用此技能。系统会保留备份和已有对话，方便恢复。' : 'The Skill will leave Harness discovery and move to the PAIMind backup area. Conversation history is not deleted; the backup can be used for recovery.'}</p><div data-paimind-skill-dialog-actions><button ref={uninstallPrimary} type="button" data-paimind-skill-button data-danger="true" disabled={busy} onClick={() => { void uninstall() }}><PaimindTrashIcon size={14} />{zh ? '带备份卸载' : 'Uninstall with backup'}</button><button type="button" data-paimind-skill-button disabled={busy} onClick={() => { setUninstallTarget(null) }}>{zh ? '保留技能' : 'Keep Skill'}</button></div></section></div>}
  </section>
}
...
function MountedSkillCenterSurface(props: SkillCenterSurfaceProps): ReactNode {
  ...
  return createPortal(<main
    ref={surface}
    data-paimind-ui-scope="skill-market" data-paimind-product-surface="skill-center"
...
export async function apply(ctx: SkillMarketClientContext): Promise<() => Promise<void>> {
  ...
  const mounted = ctx.inject([...BASE_INJECT, 'remote.paimindSkillInstaller'], scope => {
    ...
    scope.effect(() => () => { retainSkillEditorDraft(installer, null) }, 'paimind-skill-market: unsaved editor lifetime')
