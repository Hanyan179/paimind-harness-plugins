...
it('keeps late-created styles out of another module materialization and cleanup', () => {
  ...
  markHarnessClientStyle(own, '@hansen/visual-experience')
  ...
  for (const style of document.head.querySelectorAll('style[data-style-ownership-test]:not([data-plugin])')) {
    style.setAttribute('data-plugin', '@hansen/other')
  }
  for (const style of document.head.querySelectorAll('style[data-plugin="@hansen/other"]')) style.remove()
  expect(own.isConnected).toBe(true)
  expect(own).toHaveAttribute('data-plugin', '@hansen/visual-experience')
