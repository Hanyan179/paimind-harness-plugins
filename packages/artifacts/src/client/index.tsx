...
import {
  markHarnessClientStyle, contributePaimindExtension, type PaimindWorkspaceClientContext } from '@hansen/harness-compat'
import type {
...
function installStyle(): () => void {
  ...
  style.id = STYLE_ID
  style.dataset.paimindPlugin = '@hansen/artifacts'; markHarnessClientStyle(style, '@hansen/artifacts')
...
export function ArtifactPanel({ service, sidebar, bentoPreview, scope }: ArtifactPanelProps): React.JSX.Element {
  ...
  return (
    <section ref={root} data-paimind-ui-scope="artifacts" data-paimind-artifacts aria-label={zh ? '交付文件' : 'Artifacts'}>
      <header data-paimind-artifact-header>
        <div>
          <h2>{zh ? '交付文件' : 'Artifacts'}</h2>
