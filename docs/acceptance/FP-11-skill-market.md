...
1. 页面使用唯一的 `市场 / 内置 / 已安装` Scope，不保留重复 Tab；目录不依赖当前对话，当前对话可用性只作为 Harness Runtime 状态投影。
...
13. 市场按真实分类、来源和安装状态筛选，分类显示匹配数量；筛选变化后从首批结果重新展示，后续结果通过明确的继续加载动作按需进入页面。
14. 市场不显示点赞、收藏、只看收藏或收藏优先排序，也不再写入浏览器收藏状态。
