...
This ledger records migration state, historical evidence and acceptance
conclusions. It is not a package-authoring standard or selected-version source.
Current rules live in
[`../standards/plugin-authoring.md`](../standards/plugin-authoring.md), current
versions in [`../compatibility/matrix.md`](../compatibility/matrix.md), and
package identities/roles in
[`../standards/package-roles.json`](../standards/package-roles.json).
...
| FP08 | Presentation Trace | Technically Verified | R3 Product E2E Verified; v3 Bento Local Pre-acceptance; 41-slide stress trace passed locally | Repeat v3 real AI and browser cards in the shared environment; run the new Tool-specific Read Only card |
...
| FP11 | Skill Market | Technically Verified | Product E2E Verified; `bento-ppt@1.3.0` adaptive-length and registered HTML design catalog Local Pre-acceptance | Repeat explicit `bento-ppt` install/hot discovery in shared environment; run the four-Skill Walmart chain with approved real input |
...
| FP14 | Personalization | Technically Verified | Product Model E2E Pending | Run one real settings → `paimind:personalization` user-context snapshot → model-response gate; keep canonical Settings, migration, disable, responsive and rc.6 Remote-adapter regressions |
...
| FP08 | Presentation trace | Verified | R3 PPTX provenance remains verified. R9 added v3 Sidecar references and real DeepSeek chains: the baseline has 34 Facts, 8 slides and 147/147 bindings; the explicit long-deck stress card has 104 Facts, 41 slides and 275/275 bindings. KPI/chart/table bidirectional selection, refresh/restart and fail-closed loading passed locally |
...
| FP11 | Skill market | Verified | Existing native catalog verification remains. R9 explicitly installed and hot-updated Bento Skills; `bento-ppt@1.3.0` chooses 6–30 slides by decision density, permits longer decks only on explicit request, and requires a registered 16:9 HTML design. The Walmart adapter selects five priority Finelines and validates 18–30 slides. The real four-Skill business E2E is pending approved input |
...
| FP14 | Personal Center and settings | Historical Verified; superseded | The 2026-08-15 broad preference, motion and notification design passed its former gate, but is superseded. Current FP14 is Codex-inspired Personalization only: Personality, About You and Custom Instructions project a bounded user-role Context; Notifications, PAIMind motion and synthetic Memory are excluded. The new implementation passed 73 files / 224 tests, build, migration and real `3080` browser pre-acceptance; one real model/context-snapshot turn remains |
...
the then-selected Session-local scheduler facade folded official `schedule/change` events and submitted
...
former Session-local scheduler rows are all unselected, eliminating the duplicate
...
|---|---:|---:|---:|---|---|
| `dsh-better-sidebar` | `0.12.2` | `^0.1.0-rc.6` | exact npm `@deepseek-ai/dsh@0.1.0-rc.6` | Selected and Verified in R8; adapter contract `3`; upgraded from the verified R7 `0.12.1` baseline | Never follow `latest`, a caret range or a Git branch. Record a new release as Candidate and promote it only after the full isolated upgrade gate passes |
| `@huanlin/dsh-plugin-better-sidebar-plugin-office` | `0.1.0` | Declared `dsh-better-sidebar@^0.6.0`; observed metadata mismatch on selected `0.12.2` | exact npm `@deepseek-ai/dsh@0.1.0-rc.6` + `dsh-better-sidebar@0.12.2` | Selected technical viewer provider after real PPTX/XLSX composition and browser gates; peer warning retained as known risk | Keep exact; re-run real Office Viewer gates for either package upgrade and prefer a future release whose peer metadata matches the selected provider |

## 2026-08-16 baseline remediation closeout

The suite baseline remediation completed governance-first authoring rules,
identity-based retirement, role/consumer/reachability enforcement, clean
package publication, API preservation, Fast Gate and real Release Gate. The
selected version source is
[`../compatibility/matrix.md`](../compatibility/matrix.md), the sole retirement
record is [`package-retirement-ledger.md`](package-retirement-ledger.md), and
the reproducible command/browser evidence is
